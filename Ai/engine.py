...
class CognitiveEngine:
    ...
    def __init__(self, max_context: int = 128000, architecture: str = "hybrid", gamma: float = 0.509485):
        self.max_context = max_context
        self.architecture = architecture
        self.gamma = gamma if architecture == "hybrid" else 0.0
        self.CHUNK_SIZE = 128
        self.GLOBAL_PENALTY = 0.45
        self.theta = 0.581461
        self.head_safe_ratio = 0.125000
        self.tail_safe_ratio = 0.093000
        self._token_pattern = re.compile(r'(\s+)|([a-zA-Z0-9]+)|([^\sa-zA-Z0-9]+)')

    def _get_dynamic_params(self, t_obs: int, t_head_safe: int, t_tail_safe: int) -> dict:
        if t_obs <= min(t_head_safe, t_tail_safe):
            return {"alpha": 1.0, "beta": 1.0, "k1": 0.0, "k2": 0.0}

        x_half_head = max(t_head_safe / t_obs, 0.01)
        x_half_tail = max(t_tail_safe / t_obs, 0.01)
    ...
    @staticmethod
    def _find_dead_zone(slices: list, theta: float = 0.581461) -> dict:
    ...
    def calculate(self, full_text: str, t_obs: int = 0, ssm_lambda: float = None) -> dict:
        ...
        current_t_head_safe = int(actual_t_obs * self.head_safe_ratio)
        current_t_tail_safe = int(actual_t_obs * self.tail_safe_ratio)

        p = self._get_dynamic_params(actual_t_obs, current_t_head_safe, current_t_tail_safe)
        ...
        dead_zone = self._find_dead_zone(slice_records, theta=0.581461)
